...
simple-xcipher は排他的論理和を用いた転置式暗号を提供します。
...
simple-xcipher の転置先の位置計算は単純明快です。
...
```bash
gcc -Idist/include -Ldist/lib example.c -lsimplexcipher
```

## Command 

ビルド後に作成される `bin/simple-xcipher.exe` を実行することで、コマンドラインから暗号機能を利用することができます。

```bash
./bin/simple-xcipher -k 0x123
```
...
simple-xcipher released under the [MIT License](./LICENSE).
